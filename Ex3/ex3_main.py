...
def lkDemo(img_path):
    ...
    plt.imshow(img_1 - img_2, cmap='gray')
    st = time.time()
    pts, uv = opticalFlow(img_1.astype(np.float32), img_2.astype(
        np.float32), step_size=20, win_size=15)
    et = time.time()
...
def pyrLaplacianDemo(img_path):
    ...
    for i in range(lvls):
        ...
        ax[1, i].hist(lap_pyr[i].ravel(), 256, [
                      lap_pyr[i].min(), lap_pyr[i].max()])
...
def blendDemo():
    ...
    mask = cv2.cvtColor(cv2.imread('input/mask_cat.jpg'),
    ...
    cv2.imwrite('sunset_cat.png', cv2.cvtColor(
        (im_blend * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
