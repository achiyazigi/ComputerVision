...
def biliteralFilterDemo():
    ...
    print("Max Error: {}".format(
        np.abs(filtered_image_my - filtered_image_CV).max()))
    max_p = np.abs(filtered_image_CV-filtered_image_my).max()
    print('where:', np.argwhere(
        np.abs(filtered_image_CV - filtered_image_my) == max_p))
    print('shapes:', 'my:', filtered_image_my.shape,
          'cv:', filtered_image_CV.shape)
    fig, ax = plt.subplots(1, 3)
    ax[0].set_title('cv')
    ax[1].set_title('diff')
    ax[2].set_title('my')
    ax[0].imshow(filtered_image_CV)
    ax[1].imshow(np.power(filtered_image_my - filtered_image_CV, 2))
